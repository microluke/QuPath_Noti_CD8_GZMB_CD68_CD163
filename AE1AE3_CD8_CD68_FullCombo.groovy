...
runObjectClassifier(pathObjectClassifiers + "AE1AE3_CD8_CD68_OC_CellOther(15_10_IIIB).json");
...
createAnnotationsFromPixelClassifier(pathPixelClassifiers + "PC_Tumor_AE1AE3_CD8_CD68.json", 100.0, 1000.0, "SPLIT")
...
createAnnotationsFromPixelClassifier(pathPixelClassifiers + "PC_Stroma_AE1AE3_CD8_CD68.json", 100.0, 0.0)
...
for (i in getDetectionObjects().findAll{it.getPathClass() == cell}){
    ...
    if (measurement(i, m1) >= th1 && measurement(i, m2) >= th2){
            i.setPathClass(marker1and2);
    }
    
    else if (measurement(i, m1) >= th1){
